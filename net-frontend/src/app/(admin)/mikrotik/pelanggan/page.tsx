...
import { PelangganTable } from "@/components/mikrotik/pelanggan/PelangganTable";
  ...
  React.useEffect(() => {
    const fetchRouters = async () => {
      try {
        ...
        if (active.length > 0) setSelectedRouterId(active[0].id);
      } catch (error) {
    ...
    <div className="space-y-4 py-4 sm:space-y-6 sm:p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400 sm:mt-2">
          Tampilan terpadu semua pelanggan DHCP, PPPoE, dan Static
        </p>
      </div>
      ...
      {selectedRouterId && selectedRouter ? (
        <PelangganTable key={selectedRouterId} routerId={selectedRouterId} routerName={selectedRouter.name} />
      ) : activeRouters.length > 0 ? (
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-8 text-center dark:border-gray-800 dark:bg-gray-800/50">
          <p className="text-gray-500 dark:text-gray-400">
            Silakan pilih router aktif untuk melihat daftar pelanggan
          </p>
        </div>
      ) : null}
    </div>
