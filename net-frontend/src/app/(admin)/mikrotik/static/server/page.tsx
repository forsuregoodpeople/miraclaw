...
interface ServerRowProps {
  server: HotspotServer & { id?: number; disabled?: boolean; active?: boolean };
  index: number;
  startIndex: number;
  onEdit: (server: HotspotServer & { id?: number; disabled?: boolean; active?: boolean }) => void;
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function HotspotServerPage() {
  const { user } = useAuth();
  ...
  const [editingServer, setEditingServer] = useState<(HotspotServer & { id?: number; disabled?: boolean; active?: boolean }) | null>(null);
  const [formData, setFormData] = useState<Partial<HotspotServer>>({
    name: "",
    interface: "",
    address_pool: "",
    profile: "default",
  ...
  const { data: servers = [], isLoading: isLoadingServers, refetch: refetchServers } = useHotspotServers({
  ...
  const handleOpenEditModal = useCallback((server: HotspotServer & { id?: number; disabled?: boolean; active?: boolean }) => {
  ...
  const handleCreate = useCallback(async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await StaticApi.createHotspotServer(selectedRouterId, formData);
  ...
  const handleUpdate = useCallback(async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      // API might not support update, so we try create with same name (which acts as update in MikroTik)
      await StaticApi.createHotspotServer(selectedRouterId, { ...formData, name: editingServer.name });
      SweetAlert.success("Berhasil", "Hotspot Server berhasil diperbarui");
      handleCloseModal();
      smoothRefetch();
    } catch (error: any) {
      const msg = error?.response?.data?.message || error?.serverMessage || "Gagal memperbarui Hotspot Server";
      SweetAlert.error("Error", msg);
    } finally {
      setIsSaving(false);
    }
  ...
  const handleDelete = useCallback(async (serverName: string) => {
    ...
    try {
      if ("deleteHotspotServer" in StaticApi) {
        await (StaticApi as any).deleteHotspotServer(selectedRouterId, serverName);
        SweetAlert.success("Berhasil", "Hotspot Server berhasil dihapus");
        smoothRefetch();
      } else {
        SweetAlert.error("Error", "Fitur hapus server belum tersedia di API");
      }
    } catch (error: any) {
      const msg = error?.response?.data?.message || error?.serverMessage || "Gagal menghapus Hotspot Server";
      SweetAlert.error("Error", msg);
    }
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <button
            onClick={() => router.push("/mikrotik/static")}
          ...
          >
            ...
            Kembali ke Hotspot Binding
          </button>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            Manajemen Hotspot Server
          </h1>
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            Kelola Hotspot server untuk setiap interface
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px]">
            <input
              type="text"
              placeholder="Cari nama, interface, pool, profile..."
              value={searchQuery}
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-lg bg-orange-500 px-4 py-2 text-sm font-medium text-white hover:bg-orange-600 dark:bg-orange-600 dark:hover:bg-orange-700"
          >
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-base font-semibold text-gray-900 dark:text-white">
                  Hotspot Servers — {selectedRouter?.name || "-"}
                </h2>
                {isRefreshing && (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50/50 dark:border-gray-800 dark:bg-gray-800/30">
                ...
                <th className="whitespace-nowrap px-3 py-2 md:px-6 md:py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Profile</th>
            ...
            <form onSubmit={handleCreate} className="max-h-[75vh] overflow-y-auto p-6">
              <div className="grid grid-cols-1 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                  />
                  <p className="mt-1 text-xs text-gray-400">Format: 5m, 1h, 30s</p>
                </div>
              </div>
              <div className="mt-6 flex justify-end gap-3">
                ...
                <button type="submit" disabled={isSaving}
                  className="flex items-center gap-2 rounded-lg bg-orange-500 px-4 py-2 text-sm font-medium text-white hover:bg-orange-600 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-orange-600 dark:hover:bg-orange-700">
